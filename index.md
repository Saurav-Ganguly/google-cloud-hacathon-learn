...
- [[concepts/adk-graph-workflows]] — ADK 2.0 graph workflows reference: `Workflow` + `Event` data bus, FunctionNode / Agent (`mode="single_turn"`) / `JoinNode` / `RequestInput` (HITL) / nested `Workflow` node types, edge syntax (sequential / branching / parallel+join), per-node schemas, **conditional cycles**, `{simple_key}` state-templating (NOT `{Class.field}` — that doesn't exist, corrected 2026-05-26). Reference template: `test/idea_refiner/`.
- [[concepts/adk-graph-workflows-tutorial]] — single end-to-end worked example (ticket triage) exercising every graph-workflow primitive inline, plus a side example for parallel fan-out + `JoinNode`. **Has a correction notice at top** — the body code blocks predate the 2026-05-26 templating correction; use `test/idea_refiner/` as the runnable reference.
...
- `test/idea_refiner/` — **canonical annotated graph-workflow template** (Idea Refiner business-idea validator, 2026-05-26): 10 nodes / 11 edges across 4 bands, modular file layout (`agent.py`, `schemas.py`, `prompts.py`, `tracing.py`, `nodes_input.py`, `nodes_research.py`, `nodes_scoring.py`, `nodes_output.py`). Exercises every primitive from [[concepts/adk-graph-workflows]]: `FunctionNode`, `Agent (mode=single_turn)`, `JoinNode`, `RequestInput` HITL, parallel fan-out + join, **two conditional cycles** (HITL re-prompt + refinement loop, max 3), `Event(output/message/state/route=...)`, `BuiltInPlanner` on Gemini, LiteLlm deepseek alongside Gemini, `google_search` on 3 research agents, `{simple_key}` state-templating with `FunctionNode`s unpacking Pydantic outputs (the correct working pattern — `{Class.field}` does NOT exist in ADK). Best file to copy graph-workflow patterns from.
...
- [[logs/2026-05-25]] — graph-workflows docs phase: [[concepts/adk-graph-workflows]] rewritten as full reference + new [[concepts/adk-graph-workflows-tutorial]] (~720 lines, ticket-triage worked example)
- [[logs/2026-05-26]] — Idea Refiner sandbox COMPLETE: 10-node graph workflow with 2 conditional cycles; corrected `{Class.field}` templating mistake in concept docs (only `{simple_key}` from state works); new reference template `test/idea_refiner/`
